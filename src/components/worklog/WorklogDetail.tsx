...
export default function WorklogDetail({ item, onOpenReport, onOpenStock, onOpenDivergence, onClose }: WorklogDetailProps) {
  const [tab, setTab] = useState<'summary' | 'evidence' | 'priority' | 'lineage'>('summary')
  ...
  return (
    <aside className="panel p-0 sticky top-4 flex flex-col max-h-[calc(100vh-2rem)]">
      ...
      <div className="flex border-b border-line/5 text-[11.5px]">
        {(['summary', 'evidence', 'priority', 'lineage'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 ${tab === t ? 'text-slate-100 border-b-2 border-accent' : 'text-slate-500 hover:text-slate-300'}`}
          >{t}</button>
        ))}
      </div>
